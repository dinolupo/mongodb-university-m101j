...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        //
        //  num_likes
        System.out.printf("permalink:%s ordinal:%d",permalink, ordinal);
        Document post = findByPermalink(permalink);
        ArrayList<Document> comments = post.get("comments", ArrayList.class);
        System.out.println(comments.get(ordinal));
        Document toChange = comments.get(ordinal);

        postsCollection.updateOne(eq("_id", post.getObjectId("_id")), new Document("$set", new Document("comments."+ordinal+".num_likes", toChange.getInteger("num_likes")+1)));
